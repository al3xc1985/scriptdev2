...
struct MANGOS_DLL_DECL boss_lich_king_hor_endAI: public npc_escortAI
{
    ...
    void Reset()
    {
        ...
        // Jaina & Sylvanas
        if(Creature* pJaina = m_creature->GetMap()->GetCreature(m_pInstance->GetData64(NPC_JAINA_END)))
        {
            if(!pJaina->isAlive())
                pJaina->Respawn();
        }
        if(Creature* pSylvanas = m_creature->GetMap()->GetCreature(m_pInstance->GetData64(NPC_SYLVANAS_END)))
        {
    ...
    {
        if (!m_bHasTaunted && pWho->isInAccessablePlaceFor(m_creature) && !m_bIsIntro && pWho->GetTypeId() == TYPEID_PLAYER && m_creature->IsWithinDistInMap(pWho, 40) && m_creature->IsWithinLOSInMap(pWho) && m_pInstance->GetData(TYPE_GENERAL) == DONE)
        {
            m_bIsIntro = true;
            m_bHasTaunted = true;
        ...
        if (m_bIsIntro)
        {
            if(m_uiSpeech_Timer < uiDiff)
            {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    if(GameObject* pDoor = m_pInstance->instance->GetGameObject(m_pInstance->GetData64(GO_SHADOW_THROME_DOOR_EXIT)))
...
struct MANGOS_DLL_DECL npc_sylvanas_jaina_hor_endAI: public npc_escortAI
{
    ...
    bool AreMinionsDead()
    {
        if(GetClosestCreatureWithEntry(m_creature, NPC_RAGING_GHOUL, 80.0f))
            return false;

        if(GetClosestCreatureWithEntry(m_creature, NPC_RISEN_WHITCH_DOCTOR, 80.0f))
            return false;

        if(GetClosestCreatureWithEntry(m_creature, NPC_LUMBERING_ABONIMATION, 80.0f))
            return false;
...
void AddSC_boss_lich_king()
{
    ...
    pNewScript->Name = "boss_lich_king_hor_end";
    pNewScript->GetAI = &GetAI_boss_lich_king_hor_end;
    pNewScript->RegisterSelf();

    pNewScript = new Script;
    pNewScript->GetAI = &GetAI_npc_sylvanas_jaina_hor_end;
    pNewScript->Name = "npc_slyvanas_jaina_hor_end";
    pNewScript->pGossipHello = &GossipHello_npc_slyvanas_jaina_hor_end;
    pNewScript->pGossipSelect = &GossipSelect_npc_slyvanas_jaina_hor_end;
    pNewScript->RegisterSelf();
}
