...
struct MANGOS_DLL_DECL mob_hallsOfReflectionSoulAI : public ScriptedAI
{
    ...
    {
        if(uiDamage > m_creature->GetHealth() && !m_bHasCasted && creatureEntry == NPC_PHANTOM_HALLUCINATION)
        {
    ...
    {
        if(pSummon->GetEntry() == NPC_PHANTOM_HALLUCINATION)
        {
    ...
    void AttackStart(Unit* pWho)
    {
        if (m_creature->Attack(pWho, true)) 
        {
            ...
            if(m_creature->GetEntry() == NPC_PHANTOM_HALLUCINATION || m_creature->GetEntry() == NPC_PHANTOM_MAGE || m_creature->GetEntry() == NPC_GHOSTLY_PRIEST || m_creature->GetEntry() == NPC_TORTURED_RIFLEMAN)
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        case NPC_SHADOWY_MERCENARY:
            ...
            break;
        case NPC_PHANTOM_HALLUCINATION:
            ...
            if(m_uiSpeech_Timer < uiDiff)
            {
                ...
                case NPC_JAINA_START:
                    ...
                    case 20:
                        ...
                        if(GameObject* pDoor = m_pInstance->instance->GetGameObject(m_pInstance->GetData64(GO_IMPENETRABLE_DOOR)))
                    ...
                    case 28:
                        ...
                        break;
                    case 29:
                        if(GameObject* pEnterDoor = m_pInstance->instance->GetGameObject(m_pInstance->GetData64(GO_ICECROWN_DOOR)))
                            m_pInstance->DoUseDoorOrButton(pEnterDoor->GetGUID());
                        if(GameObject* pExitDoor = m_pInstance->instance->GetGameObject(m_pInstance->GetData64(GO_IMPENETRABLE_DOOR)))
...
void AddSC_halls_of_reflection()
{
    ...
    pNewScript->Name = "mob_hallsOfReflectionSoul";
    pNewScript->GetAI = &GetAI_mob_hallsOfReflectionSoul;
    pNewScript->RegisterSelf();

    pNewScript = new Script;
    pNewScript->Name = "npc_lich_king_hor_start";
    pNewScript->GetAI = &GetAI_npc_lich_king_hor_start;
    pNewScript->RegisterSelf();

    pNewScript = new Script;
    pNewScript->GetAI = &GetAI_npc_sylvanas_jaina_hor_start;
    pNewScript->Name = "npc_slyvanas_jaina_hor_start";
    pNewScript->pGossipHello = &GossipHello_npc_slyvanas_jaina_hor_start;
    pNewScript->pGossipSelect = &GossipSelect_npc_slyvanas_jaina_hor_start;
    pNewScript->RegisterSelf();
}
