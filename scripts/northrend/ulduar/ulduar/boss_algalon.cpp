...
struct MANGOS_DLL_DECL boss_algalonAI : public ScriptedAI
{
    ...
    void DoOutro()
    {
        if (m_pInstance)
        {
            ...
            m_pInstance->DoUpdateWorldState(WORLD_STATE_TIMER, 0);
    ...
    void Aggro(Unit* pWho)
    {
        ...
        if(m_bIsFirstTime)
        {
            ...
            if(m_pInstance)
            {
                ...
                m_pInstance->DoUpdateWorldState(WORLD_STATE_TIMER_COUNT, m_uiDespawnTimer / 60000);
            }
        }
        else
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if(m_uiDespawnTimer < m_uiLastTimer - 60000 && !m_bIsDespawned)
        {
            ...
            if(m_pInstance)
                m_pInstance->DoUpdateWorldState(WORLD_STATE_TIMER_COUNT, tMinutes);
        }
