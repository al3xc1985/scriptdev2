...
{   
    ...
    void KilledUnit(Unit* pVictim)
    {
        if (Creature* pMimiron = m_creature->GetMap()->GetCreature(m_pInstance->GetData64(NPC_MIMIRON)))
        {
            ...
            {
                if(irand(0,1))
                    DoScriptText(SAY_TANK_SLAY_1, pMimiron);
                else
                    DoScriptText(SAY_TANK_SLAY_2, pMimiron);
            }
            if(m_pInstance->GetData(TYPE_MIMIRON_PHASE) == PHASE_ROBOT)
            {
                if(irand(0,1))
                    DoScriptText(SAY_ROBOT_SLAY_1, pMimiron);
                else
                    DoScriptText(SAY_ROBOT_SLAY_2, pMimiron);
            }
        }
    }
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        if(!m_bIsOutro)
        {
            ...
            // this should be removed when vehicles are implemented!
            if(m_pInstance->GetData(TYPE_MIMIRON_PHASE) == PHASE_LEVIATHAN)
            {
...
{   
    ...
    void KilledUnit(Unit* pVictim)
    {
        if (Creature* pMimiron = m_creature->GetMap()->GetCreature(m_pInstance->GetData64(NPC_MIMIRON)))
        {
            ...
            {
                if(irand(0,1))
                    DoScriptText(SAY_TORSO_SLAY_1, pMimiron);
                else
                    DoScriptText(SAY_TORSO_SLAY_2, pMimiron);
            }
            if(m_pInstance->GetData(TYPE_MIMIRON_PHASE) == PHASE_ROBOT)
            {
                if(irand(0,1))
...
{   
    ...
    void KilledUnit(Unit* pVictim)
    {
        if (Creature* pMimiron = m_creature->GetMap()->GetCreature(m_pInstance->GetData64(NPC_MIMIRON)))
        {
            ...
            {
                if(irand(0,1))
                    DoScriptText(SAY_HEAD_SLAY_1, pMimiron);
                else
                    DoScriptText(SAY_HEAD_SLAY_2, pMimiron);
            }
            if(m_pInstance->GetData(TYPE_MIMIRON_PHASE) == PHASE_ROBOT)
            {
                if(irand(0,1))
